...
import type { Attachment, ServiceJob } from '../../lib/types'
import {
  IconChevronDown,
  IconDownload,
  IconFileText,
...
interface JobAttachmentModalProps {
  job: ServiceJob
  canManage: boolean
  onClose: () => void
  onSaved: () => void
...
function JobAttachmentModal({
  job,
  canManage,
  onClose,
  onSaved,
}: JobAttachmentModalProps) {
  ...
  async function reload() {
    try {
      const fresh = await apiFetch<ServiceJob>(`/api/service-jobs/${job.jobNo}`)
  ...
  async function togglePreview(attachment: Attachment) {
    ...
    try {
      const blob = await apiFetchBlob(
        `/api/service-jobs/${job.jobNo}/attachments/download?ids=${attachment.id}`,
      )
  ...
  async function handleUpload() {
    ...
    try {
      ...
      await apiFetch(`/api/service-jobs/${job.jobNo}/attachments`, {
        method: 'POST',
        body: formData,
      })
      setPendingFiles([])
  ...
  async function handleDownload() {
    ...
    try {
      ...
      const blob = await apiFetchBlob(
        `/api/service-jobs/${job.jobNo}/attachments/download${idsQuery}`,
      )
      const targetCount =
  ...
  async function handleDeleteAttachment(attachment: Attachment) {
    ...
    try {
      await apiFetch(
        `/api/service-jobs/${job.jobNo}/attachments/${attachment.id}`,
        {
          method: 'DELETE',
        },
      )
  ...
  return (
    <Modal title={`ไฟล์แนบ — ${job.jobNo}`} onClose={onClose} size="lg">
