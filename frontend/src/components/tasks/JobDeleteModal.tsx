...
function JobDeleteModal({ job, onClose, onDeleted }: JobDeleteModalProps) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      await apiFetch(`/api/service-jobs/${job.jobNo}`, { method: 'DELETE' })
      onDeleted()
      onClose()
    } catch (err) {
  ...
  return (
    <Modal title="ยืนยันการลบ" onClose={onClose}>
      <p className="text-sm text-gray-600 dark:text-gray-300">
        ต้องการลบงาน <span className="font-medium text-gray-800 dark:text-gray-100">{job.jobNo}</span> —{' '}
        {job.title} ใช่หรือไม่? การลบไม่สามารถย้อนกลับได้
      </p>
